...
`Blue_Marble-frontend`에서 AI가 작업할 때 따라야 하는 최소 운영 규칙이다.
목표는 "대화 기억"이 아니라 "문서와 검증 기준"에 따라 일하게 만드는 것이다.
...
3. 작업이 여러 단계면 `docs/ai/tasks/<task-slug>/` 아래에 `plan.md`, `context.md`, `checklist.md`를 만들거나 갱신한다.
4. 기존 계층을 유지한 채 구현한다.
5. 가장 좁은 테스트부터 실행하고, 영향 범위에 맞는 추가 검증을 수행한다.
6. 결과 보고 시 변경 파일, 검증 결과, 미해결 리스크를 남긴다.
